...
contract('Marketplace', ([deployer, seller, buyer]) => {
  ...
  describe('product', async () => {
    ...
    it('create successfully', async () => {
        ...
        await marketplace.createProduct('Phone',0, { from: seller }).should.be.rejected;
    })

    it('lists products', async () => {
        const product = await marketplace.products(productCount);
        assert.equal(product.id.toNumber(), productCount, "id is correct");
        assert.equal(product.name, "Phone", "name is correct");
    ...
    it('sells products', async () => {
        ...
        await marketplace.purchaseProduct(productCount, { from: buyer, value: 0 }).should.be.rejected;
